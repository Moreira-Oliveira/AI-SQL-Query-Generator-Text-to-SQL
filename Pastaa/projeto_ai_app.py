...
def projeto_download_resultado(sql_query, e_output, explicacao):
    ...
    st.download_button("Baixar Resultado", conteudo, file_name = "projeto_resultado_query.sql", mime = "text/plain")

# Função principal
def main():

    # Configuração da página
    st.set_page_config(page_title="Projeto AI SQ", page_icon=":100:", layout="wide")

    # Sidebar com instruções
    with st.sidebar:
        st.header("Projeto AI SQL")
        ...
        if st.button("Suporte"):
            st.write("Dúvidas? Envie um e-mail para: moreira-tra@hotmail.com")

    # Título principal
    st.markdown(
        """
        <div style="text-align: center;">
            <h1>projeto AI SQL Query Generator</h1>
            <h2>Text-to-SQL</h2>
    ...
    if submit:
        ...
        with st.spinner("A IA está gerando o resultado. Aguarde..."):
            ...
            sql_query = projeto_gera_resposta_modelo(f"Crie de forma clara, objetiva e profissional, uma consulta SQL baseada neste texto: {text_input}")

            # Se a primeira resposta foi gerada, seguimos usando IA para gerar as demais saídas 
            if sql_query:

                # Exemplo de saída da query
                e_output = projeto_gera_resposta_modelo(f"Mostre um exemplo da saída esperada para: {sql_query}")

                # Explicação
                explicacao = projeto_gera_resposta_modelo(f"Avalie e detalhe a explicação da sintaxe desta consulta SQL, descrevendo cada cláusula e função utilizada: {sql_query}")
                ...
                # Opção de download
                projeto_download_resultado(sql_query, e_output, explicacao)
